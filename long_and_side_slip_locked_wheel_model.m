function dxdt = long_and_side_slip_locked_wheel_model(x, omega_l, omega_r, params)
    theta = x(3);    
    ...
    if(omega_input == 0.0) % Moving straight
       dxdt = [v_input*cos(theta); v_input*sin(theta); 0.0];
        ...
        b0 = params.locked_wheel_coeff(1);
        b1 = params.locked_wheel_coeff(2);
        
        if(i_L > 1.0) % full slip
            v_wheel_l = 0.0;
        elseif(i_L < params.slip_fit_coeff.min_value) % locked track
            v_wheel_l = b0 + b1 * v_input;
